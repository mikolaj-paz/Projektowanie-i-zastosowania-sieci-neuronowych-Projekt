...
def image_output_from_model(model: nn.Module, device: torch.device, img: Image):
    ...
    tensor: torch.Tensor = transforms.ToTensor()(img).to(device)

    if type(model) is SRCNN:
        tensor = nn.Upsample(scale_factor=2, mode='bicubic')(tensor.unsqueeze(0))
...
def visualize_all_models(srcnn_src: str, device: torch.device, hr: Image, lr: Image):
...
if __name__ == '__main__':
    ...
    visualize_all_models(args.srcnn, device, hr_img, lr_img)